...
value,reason);break;case "selectedValues":null==onChange?void 0:onChange(event,value,reason)}},[handleHighlightChange,onChange,onStateChange]);const listActionContext=React.useMemo(()=>({disabledItemsFocusable,disableListWrap,focusManagement,isItemDisabled,itemComparer,items,getItemAsString,onHighlightChange:handleHighlightChange,orientation,pageSize,selectionMode,stateComparers}),[disabledItemsFocusable,disableListWrap,focusManagement,isItemDisabled,itemComparer,items,getItemAsString,handleHighlightChange,
...
void 0:_other$onKeyDown.call(other,event);event.defaultMuiPrevented||(_other$onKeyDown=["Home","End","PageUp","PageDown"],"vertical"===orientation?_other$onKeyDown.push("ArrowUp","ArrowDown"):_other$onKeyDown.push("ArrowLeft","ArrowRight"),"activeDescendant"===focusManagement&&_other$onKeyDown.push(" ","Enter"),_other$onKeyDown.includes(event.key)&&event.preventDefault(),dispatch({type:_listActions.ListActionTypes.keyDown,key:event.key,event}),handleTextNavigation(event))},createHandleBlur=other=>
event=>{var _other$onBlur,_listRef$current;null==(_other$onBlur=other.onBlur)?void 0:_other$onBlur.call(other,event);event.defaultMuiPrevented||null!=(_listRef$current=listRef.current)&&_listRef$current.contains(event.relatedTarget)||dispatch({type:_listActions.ListActionTypes.blur,event})},getItemState=React.useCallback(item=>{var _latestSelectedValues;const index=items.findIndex(i=>itemComparer(i,item)),selected=(null!=(_latestSelectedValues=latestSelectedValues.current)?_latestSelectedValues:[]).some(value=>
