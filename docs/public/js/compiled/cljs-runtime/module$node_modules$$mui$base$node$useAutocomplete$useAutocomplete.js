...
newValue="":null==newValue?newValue="":(newValue=getOptionLabel(newValue),newValue="string"===typeof newValue?newValue:""),inputValue!==newValue&&(setInputValueState(newValue),onInputChange&&onInputChange(event,newValue,"reset"))},[getOptionLabel,inputValue,multiple,onInputChange,setInputValueState,clearOnBlur,value$jscomp$0]),[open,setOpenState]=(0,_utils.unstable_useControlled)({controlled:openProp,default:!1,name:componentName,state:"open"}),[inputPristine,setInputPristine]=React.useState(!0),
inputValueIsSelectedValue=!multiple&&null!=value$jscomp$0&&inputValue===getOptionLabel(value$jscomp$0),popupOpen=open&&!readOnly,filteredOptions=popupOpen?filterOptions(options.filter(option=>filterSelectedOptions&&(multiple?value$jscomp$0:[value$jscomp$0]).some(value2=>null!==value2&&isOptionEqualToValue(option,value2))?!1:!0),{inputValue:inputValueIsSelectedValue&&inputPristine?"":inputValue,getOptionLabel}):[],previousProps=(0,_utils.usePreviousProps)({filteredOptions,value:value$jscomp$0});React.useEffect(()=>
{const valueChange=value$jscomp$0!==previousProps.value;focused&&!valueChange||freeSolo&&!valueChange||resetInputValue(null,value$jscomp$0)},[value$jscomp$0,resetInputValue,focused,previousProps.value,freeSolo]);const listboxAvailable=open&&0<filteredOptions.length&&!readOnly;null!==value$jscomp$0&&!freeSolo&&0<options.length&&(props=(multiple?value$jscomp$0:[value$jscomp$0]).filter(value2=>!options.some(option=>isOptionEqualToValue(option,value2))),0<props.length&&console.warn([`MUI: The value provided to ${componentName} is invalid.`,
...
event.classList.remove(`${unstable_classNamePrefix}-focusVisible`);event=listboxRef.current;"listbox"!==listboxRef.current.getAttribute("role")&&(event=listboxRef.current.parentElement.querySelector('[role\x3d"listbox"]'));if(event)if(-1===index)event.scrollTop=0;else if(index=listboxRef.current.querySelector(`[data-option-index="${index}"]`))index.classList.add(`${unstable_classNamePrefix}-focused`),"keyboard"===reason&&index.classList.add(`${unstable_classNamePrefix}-focusVisible`),event.scrollHeight>
event.clientHeight&&"mouse"!==reason&&(reason=index.offsetTop+index.offsetHeight,reason>event.clientHeight+event.scrollTop?event.scrollTop=reason-event.clientHeight:index.offsetTop-index.offsetHeight*(groupBy?1.3:0)<event.scrollTop&&(event.scrollTop=index.offsetTop-index.offsetHeight*(groupBy?1.3:0)))}}),changeHighlightedIndex=(0,_utils.unstable_useEventCallback)(({event,diff,direction="next",reason="auto"})=>{popupOpen&&(direction=validOptionIndex((()=>{const maxIndex=filteredOptions.length-1;if("reset"===
diff)return defaultHighlighted;if("start"===diff)return 0;if("end"===diff)return maxIndex;const newIndex=highlightedIndexRef.current+diff;return 0>newIndex?-1===newIndex&&includeInputInList?-1:disableListWrap&&-1!==highlightedIndexRef.current||1<Math.abs(diff)?0:maxIndex:newIndex>maxIndex?newIndex===maxIndex+1&&includeInputInList?-1:disableListWrap||1<Math.abs(diff)?maxIndex:0:newIndex})(),direction),setHighlightedIndex({index:direction,reason,event}),autoComplete&&"reset"!==diff&&(-1===direction?
inputRef.current.value=inputValue:(event=getOptionLabel(filteredOptions[direction]),inputRef.current.value=event,0===event.toLowerCase().indexOf(inputValue.toLowerCase())&&0<inputValue.length&&inputRef.current.setSelectionRange(inputValue.length,event.length))))}),checkHighlightedOptionExists=()=>{const isSameValue=(value1,value2)=>{value1=value1?getOptionLabel(value1):"";value2=value2?getOptionLabel(value2):"";return value1===value2};if(-1!==highlightedIndexRef.current&&previousProps.filteredOptions&&
previousProps.filteredOptions.length!==filteredOptions.length&&(multiple?value$jscomp$0.length===previousProps.value.length&&previousProps.value.every((val,i)=>getOptionLabel(value$jscomp$0[i])===getOptionLabel(val)):isSameValue(previousProps.value,value$jscomp$0))){const previousHighlightedOption=previousProps.filteredOptions[highlightedIndexRef.current];if(previousHighlightedOption&&filteredOptions.some(option=>getOptionLabel(option)===getOptionLabel(previousHighlightedOption)))return!0}return!1},
syncHighlightedIndex=React.useCallback(()=>{if(popupOpen&&!checkHighlightedOptionExists()){var valueItem=multiple?value$jscomp$0[0]:value$jscomp$0;if(0===filteredOptions.length||null==valueItem)changeHighlightedIndex({diff:"reset"});else if(listboxRef.current)if(null!=valueItem){const currentOption=filteredOptions[highlightedIndexRef.current];if(!multiple||!currentOption||-1===findIndex(value$jscomp$0,val=>isOptionEqualToValue(currentOption,val))){var itemIndex=findIndex(filteredOptions,optionItem=>
isOptionEqualToValue(optionItem,valueItem));-1===itemIndex?changeHighlightedIndex({diff:"reset"}):setHighlightedIndex({index:itemIndex})}}else highlightedIndexRef.current>=filteredOptions.length-1?setHighlightedIndex({index:filteredOptions.length-1}):setHighlightedIndex({index:highlightedIndexRef.current})}},[filteredOptions.length,multiple?!1:value$jscomp$0,filterSelectedOptions,changeHighlightedIndex,setHighlightedIndex,popupOpen,inputValue,multiple]),handleListboxRef=(0,_utils.unstable_useEventCallback)(node=>
...
"clear"):handleOpen(event)},handleOptionMouseMove=event=>{const index=Number(event.currentTarget.getAttribute("data-option-index"));highlightedIndexRef.current!==index&&setHighlightedIndex({event,index,reason:"mouse"})},handleOptionTouchStart=event=>{setHighlightedIndex({event,index:Number(event.currentTarget.getAttribute("data-option-index")),reason:"touch"});isTouch.current=!0},handleOptionClick=event=>{const index=Number(event.currentTarget.getAttribute("data-option-index"));selectNewValue(event,
filteredOptions[index],"selectOption");isTouch.current=!1},handleTagDelete=index=>event=>{const newValue=value$jscomp$0.slice();newValue.splice(index,1);handleValue(event,newValue,"removeOption",{option:value$jscomp$0[index]})},handlePopupIndicator=event=>{open?handleClose(event,"toggleInput"):handleOpen(event)},handleMouseDown=event=>{event.currentTarget.contains(event.target)&&event.target.getAttribute("id")!==id&&event.preventDefault()},handleClick=event=>{event.currentTarget.contains(event.target)&&
(inputRef.current.focus(),selectOnFocus&&firstFocus.current&&0===inputRef.current.selectionEnd-inputRef.current.selectionStart&&inputRef.current.select(),firstFocus.current=!1)},handleInputMouseDown=event=>{""!==inputValue&&open||handlePopupIndicator(event)};props=(props=freeSolo&&0<inputValue.length)||(multiple?0<value$jscomp$0.length:null!==value$jscomp$0);let groupedOptions=filteredOptions;if(groupBy){const indexBy=new Map;let warn=!1;groupedOptions=filteredOptions.reduce((acc,option,index)=>{const group=
groupBy(option);0<acc.length&&acc[acc.length-1].group===group?acc[acc.length-1].options.push(option):(indexBy.get(group)&&!warn&&(console.warn(`MUI: The options provided combined with the \`groupBy\` method of ${componentName} returns duplicated headers.`,"You can solve the issue by sorting the options with the output of `groupBy`."),warn=!0),indexBy.set(group,!0),acc.push({key:index,index,group,options:[option]}));return acc},[])}disabledProp&&focused&&handleBlur();return{getRootProps:(other={})=>
(0,_extends2.default)({"aria-owns":listboxAvailable?`${id}-listbox`:null},other,{onKeyDown:handleKeyDown(other),onMouseDown:handleMouseDown,onClick:handleClick}),getInputLabelProps:()=>({id:`${id}-label`,htmlFor:id}),getInputProps:()=>({id,value:inputValue,onBlur:handleBlur,onFocus:handleFocus,onChange:handleInputChange,onMouseDown:handleInputMouseDown,"aria-activedescendant":popupOpen?"":null,"aria-autocomplete":autoComplete?"both":"list","aria-controls":listboxAvailable?`${id}-listbox`:void 0,"aria-expanded":listboxAvailable,
autoComplete:"off",ref:inputRef,autoCapitalize:"none",spellCheck:"false",role:"combobox",disabled:disabledProp}),getClearProps:()=>({tabIndex:-1,onClick:handleClear}),getPopupIndicatorProps:()=>({tabIndex:-1,onClick:handlePopupIndicator}),getTagProps:({index})=>(0,_extends2.default)({key:index,"data-tag-index":index,tabIndex:-1},!readOnly&&{onDelete:handleTagDelete(index)}),getListboxProps:()=>({role:"listbox",id:`${id}-listbox`,"aria-labelledby":`${id}-label`,ref:handleListboxRef,onMouseDown:event=>
{event.preventDefault()}}),getOptionProps:({index,option})=>{const selected=(multiple?value$jscomp$0:[value$jscomp$0]).some(value2=>null!=value2&&isOptionEqualToValue(option,value2)),disabled=getOptionDisabled?getOptionDisabled(option):!1;return{key:getOptionLabel(option),tabIndex:-1,role:"option",id:`${id}-option-${index}`,onMouseMove:handleOptionMouseMove,onClick:handleOptionClick,onTouchStart:handleOptionTouchStart,"data-option-index":index,"aria-disabled":disabled,"aria-selected":selected}},id,
inputValue,value:value$jscomp$0,dirty:props,expanded:popupOpen&&anchorEl,popupOpen,focused:focused||-1!==focusedTag,anchorEl,setAnchorEl,focusedTag,groupedOptions}};var _extends2=global(require("module$node_modules$$babel$runtime$helpers$extends")),React=function(obj,nodeInterop){if(!nodeInterop&&obj&&obj.__esModule)return obj;if(null===obj||"object"!==typeof obj&&"function"!==typeof obj)return{default:obj};if((nodeInterop=_getRequireWildcardCache(nodeInterop))&&nodeInterop.has(obj))return nodeInterop.get(obj);
...
const defaultFilterOptions=createFilterOptions(),defaultIsActiveElementInListbox=listboxRef=>{var _listboxRef$current$p;return null!==listboxRef.current&&(null==(_listboxRef$current$p=listboxRef.current.parentElement)?void 0:_listboxRef$current$p.contains(document.activeElement))}}
